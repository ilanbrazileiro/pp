...
class Usuario extends Model {
	...
	*/
	public static function login($login, $password){
		...
		$sql = new Sql();
		$results = $sql->select("SELECT * FROM usuarios WHERE email = :EMAIL OR login = :LOGIN", array(":EMAIL"=>$login,":LOGIN"=>$login)); 
	...
	public static function verifyLogin($check = false){
		...
		if (!Usuario::checkLogin()) {
			...
			return $usuarios->getFromSession();
		}
	...
	*/
	...
	/*
		Retorna true se existir sessão do Usuario e false caso não exista
	*/
	...
	*/
	public function listarTodos(){
		$sql = new Sql();
		$results = $sql->select("SELECT * FROM usuarios WHERE login != 'root' ORDER BY id_usuario DESC");
		return (array)$results;
	}

	/*
		Retorna password hash
	*/
	...
	public static function getNameSession(){
		return $_SESSION[Usuario::SESSION]["nome"];
	}

	public static function getFuncaoSession(){
		return $_SESSION[Usuario::SESSION]['funcao'];
	}

	public static function checkAdmin(){
		if (Usuario::getFuncaoSession() == 'admin'){
			return true;
		} else {
			return false;
		}
	...
	public function adicionar($data){
		...
		//insere no banco
		$result = $sql->query("INSERT INTO usuarios (
							nome,
							login,
							email,
							celular,
	...
	public function alterar($data){
		...
		//insere no banco
		$result = $sql->query("UPDATE usuarios SET 
			nome = '".$data['nome']."',	
			login = '".$data['login']."', 
			email = '".$data['email']."',
			...
			Model::setSuccess('Usuário alterado com sucesso!');
			return true;		
	}// FIM ALTERAR
	...
	public function alterarSenha($data){
		...
		$sql = new Sql();
		$results = $sql->select("SELECT * FROM usuarios WHERE id_usuario = '".$data['id_usuario']."'"); 
		...
		if (password_verify($data['senha_atual'], $results[0]["senha"]) === true){

			$senha = $this->getPasswordHash($data['nova_senha']);

			$result = $sql->query("UPDATE usuarios SET senha = '".$senha."'	WHERE id_usuario = ".$data['id_usuario']);
		
			Model::setSuccess('Senha alterada com sucesso!');
			return true;		

		} else {
			Model::setError('Senha inválida');
			return false ;
		}
	}
	...
	public function deletar($id_usuario){
		$sql = new Sql();
		$result = $sql->query("DELETE FROM usuarios WHERE login != 'root' AND id_usuario = ".$id_usuario);
		if ($result === true){
			Model::setSuccess('Usuário deletado com sucesso!');	
		}
	}
	...
	{
		$sql = new Sql();
		$result = $sql->select("SELECT * FROM usuarios WHERE id_usuario = '$id_usuario'");
		return $result[0];
	}
	...
	public function getUsuario(){
		...
		$sql = new Sql();
		$result = $sql->select("SELECT * FROM usuarios WHERE id_usuario = '".$_SESSION[Usuario::SESSION]["id_usuario"]."'");
		//return $result[0];
		$this->setData($result[0]);

	}

	public static function getNomeUsuario($id_usuario){
		$sql = new Sql();
		$result = (array)$sql->select("SELECT nome FROM usuarios WHERE id_usuario = '$id_usuario'");
	...
	public static function getFromSession(){
		...
		if (isset($_SESSION[Usuario::SESSION]) && (int)$_SESSION[Usuario::SESSION]['id_usuario'] > 0) {
			...
			$usuarios->setData($_SESSION[Usuario::SESSION]);
		}
		return $usuarios;
	}
