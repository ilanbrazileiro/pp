...
class Cursos extends Model {
	...
	*/
	public function listarTodos(){
		$sql = new Sql();
		$results = $sql->select("SELECT * FROM curso ORDER BY id_curso DESC");
		return (array)$results;
	}

	public static function getCursos(){
		$sql = new Sql();
		$results = $sql->select("SELECT id_curso, titulo FROM curso ORDER BY titulo ASC");
		$data = array();
	...
	{
		$sql = new Sql();
		$result = $sql->select("SELECT * FROM curso WHERE id_curso = '$id_curso'");
		return $result[0];
	}
	...
	public function adicionar($data){
		...
		//insere no banco
		$result = $sql->query("INSERT INTO curso (
							titulo,
							valor,
							slug,
							descricao,
		...
		if ($result === true){
			Model::setSuccess('Curso cadastrado com sucesso!');
			return true;
		} else {
			Model::setError('Ocorreu um erro ao cadastrar este curso, informe ao administrador do sistema o erro abaixo: <br>'.$result[2]);
			return false ;
		}
...
############## DEEPOIS CRIAR ROTINAA PaRA NÃO DELETAR ENQUANTO HOUVER DISCIPLINA LIGADA AO CURSO
		$result = $sql->query("DELETE FROM curso WHERE id_curso = ".$id_curso);
		if ($result === true){
	...
	public function alterar($data){
		...
		$sql = new Sql();
		$result = $sql->query("UPDATE curso SET 
			titulo = '".$data['titulo']."',	
			valor = '".$data['valor']."', 
			slug = '".$data['slug']."', 
		...
		if ($result == true){
			Model::setSuccess('Curso alterado com sucesso!');
			return true;
		} else {
			return $result;
		}
...
/*
	...
	public function adicionar($data){
		...
		//insere no banco
		$result = $sql->query($sql->geraQueryInsert('usuarios', $data));

		
		if ($result == true){
			//busca o ultimo adicionado
			$novo_usuario = $sql->select("SELECT * FROM usuarios WHERE id_usuario = LAST_INSERT_ID()");
	...
	public function getUserDTCadastro(){//retorna o login de quem está logado

		$data = $_SESSION[Usuario::SESSION]['dt_cadastro'];

		return $data;

	}

	public function getUserfuncao(){//retorna o login de quem está logado

		$data = $_SESSION[Usuario::SESSION]['funcao'];

		return $data;

	}

	public function getUserId(){//retorna o login de quem está logado

		$data = $_SESSION[Usuario::SESSION]['id_usuario'];
